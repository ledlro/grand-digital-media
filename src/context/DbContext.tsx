...
const DEFAULT_STAFF: Staff[] = [
  ...
  { id: 'STF-0006', name: 'System Administrator', username: 'admin', role: 'Admin', phone: '9000000000', email: 'admin@granddigitalmedia.in', status: 'Active', password: 'admin123' }
];
...
export const DbProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const login = async (username: string, password: string): Promise<{ success: boolean; message: string; user?: Staff }> => {
    ...
    const expectedPassword = user.password || (user.username === 'admin' ? 'admin123' : `${user.username}123`);
